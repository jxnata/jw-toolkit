...
const useAllMaps = (props: Props = { search: '', district: '', status: '', city: '', initialData: [] }) => {
	const { search = '', district = '', status = '', city = '', initialData = [] } = props
	const congregation = storage.getString('congregation.id')
	const queryKey = ['all-maps', search, city, district, status, congregation]
