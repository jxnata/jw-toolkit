...
const ListItem = ({ id, name, onPress, label, count }: ListItemProps) => {
	return (
		<Pressable key={id} onPress={onPress} className="h-[65px] flex-row items-center rounded-lg bg-card px-4 py-2">
			...
			<View className="flex-1 flex-row items-center justify-between">
				<View>
					...
					{label && (
						<View className="mt-1 flex-row">
							<View className={`${label.color} rounded-xl px-2 py-1`}>
				...
				{typeof count === 'number' && (
					<Text className="font-medium text-sm text-foreground opacity-70">
